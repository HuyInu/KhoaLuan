...
@section('head')
    @include('Admin.QLHaTangKyThuat.CapDien.DuongDayDien.head')
@endsection
    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex align-items-center">
                <h4 class="card-title">Quản lý đường dây điện</h4>
            </div>
        </div>
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="LoaiDuongDien_Sort">
                    <option value="" selected hidden disablded>Chọn Loai đường dây điện</option>
                    <option value=''>Tất cả</option>
                    @foreach($LoaiDuongDayDien_list as $item)
                    <option value="{{$item->TenLoaiDuongDayDien}}">{{$item->TenLoaiDuongDayDien}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label for="defaultSelect">Dự án quy hoạch:</label>
                <select class="form-control" id="DAQH_DuongDayDien_Sort"  >
            ...
            <div class="table-responsive">
                <table id="table" class="display table table-striped table-hover" >
                    <thead>
                        <tr>
                            <th>Loại đường dây điện</th>
                            <th>Tên đường dây điện</th>
                    ...
                    <tbody>
                        @foreach($DuongDayDien_List as $item)
                        <tr>
                            <td>{{$item->LoaiDuongDayDien->TenLoaiDuongDayDien ?? ''}}</td>
                            <td>{{$item->Ten}}</td>
...
@section('footer')
    @include('Admin.QLHaTangKyThuat.CapDien.DuongDayDien.foot') 
@endsection